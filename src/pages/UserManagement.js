...
const UserManagement = () => {
  ...
  const columns = [
    ...
    { 
      ...
      render: (value) => (
        <Badge 
          variant={value === 'Admin' ? 'primary' : value === 'Manager' ? 'secondary' : 'default'}
        >
          {value}
        </Badge>
      )
    ...
    { 
      ...
      render: (value) => (
        <div className="flex items-center">
          {value === 'Active' && <CheckCircle className="w-4 h-4 text-green-500 mr-2" />}
          {value === 'Inactive' && <XCircle className="w-4 h-4 text-red-500 mr-2" />}
          {value === 'Pending' && <div className="w-4 h-4 bg-yellow-500 rounded-full mr-2" />}
  ...
  const stats = [
    ...
    { label: 'Pending Users', value: users.filter(u => u.status === 'Pending').length, color: 'yellow' },
      ...
      {showAddUser && (
        <div className="fixed inset-0 z-50 overflow-y-auto">
          <div className="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            <div className="fixed inset-0 transition-opacity bg-gray-500 bg-opacity-75" onClick={() => setShowAddUser(false)} />
            ...
            <div className="inline-block px-4 pt-5 pb-4 overflow-hidden text-left align-bottom transition-all transform bg-white rounded-lg shadow-xl sm:my-8 sm:align-middle sm:max-w-lg sm:w-full sm:p-6">
              ...
              >
                ...
                <FormField name="role" label="Role" required>
                  <FormSelect name="role" options={roleOptions} placeholder="Select role" />
                </FormField>

                <FormField name="department" label="Department" required>
                  <FormSelect name="department" options={departmentOptions} placeholder="Select department" />
                </FormField>
